...
function generateImports(
  ...
  let importCode = `from peargent import ${imports.join(", ")}
...
function getModelCode(
  model: { type: string; model_name: string } | undefined,
): string {
...
function generateToolCode(tool: ToolData & { function_body?: string }): string {
  ...
  let functionBody =
    tool.function_body || extractFunctionBody(tool.source_code);
...
export function generatePythonCode(
  pearData: PearData,
  className: string = "GeneratedAgentSystem",
): string {
...
${generateImports(hasRouter, hasHistory, routerType, coreToolNames)}
...
`;
  ...
  if (hasHistory && poolData.history) {
    const storeType =
      poolData.history.store_type?.toLowerCase().replace("historystore", "") ||
      "sqlite";
  ...
  agents.forEach((agent, index) => {
    ...
    code += "\n";
    agentVarNames.push(
      `agent_${agent.name.toLowerCase().replace(/[^a-z0-9]/g, "_")}`,
    );
  });
